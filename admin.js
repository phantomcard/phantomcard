(function () {
    ...
    async function loadView(view, { silent = false } = {}) {
        ...
        const titles = { overview: 'Overview', users: 'Users', cards: 'Cards', purchases: 'Purchases', deposits: 'Deposits', transactions: 'Transactions', withdrawals: 'Withdrawals', reconciliation: 'Payment Reconciliation', 'wallet-credits': 'Manual Wallet Credit', kyc: 'KYC Review', redemptions: 'Redemptions', audit: 'Audit Logs', settings: 'Settings' };
    ...
    function render(view) {
        const renderers = { overview: renderOverview, users: renderUsers, cards: renderCards, purchases: renderPurchases, deposits: renderDeposits, transactions: renderTransactions, withdrawals: renderWithdrawals, kyc: renderKyc, redemptions: renderRedemptions, audit: renderAudit, settings: renderSettings, reconciliation: renderReconciliation, 'wallet-credits': renderWalletCredits };
        $('#pageContent').innerHTML = renderers[view] ? renderers[view]() : renderOverview();
    }

    function renderOverview() {
        const data = state.summary; const m = data.metrics; const max = Math.max(1, ...data.trend.flatMap(item => [item.deposits, item.purchases, item.redemptions]));
        const chart = data.trend.map(item => `<div class="trend-day"><span class="trend-bar" style="height:${Math.max(2, item.deposits / max * 100)}%" title="Deposits ${money(item.deposits)}"></span><span class="trend-bar purchase" style="height:${Math.max(2, item.purchases / max * 100)}%" title="Purchases ${money(item.purchases)}"></span><span class="trend-bar redemption" style="height:${Math.max(2, item.redemptions / max * 100)}%" title="Redemptions ${money(item.redemptions)}"></span><small class="trend-label">${escape(shortDate(item.date))}</small></div>`).join('');
        return pageHeading('Overview', 'Mission control for marketplace, money movement, and verification.') + `<div class="metric-grid">${metric('Users', m.users, 'users', 'Registered accounts')}${metric('Active cards', m.activeCards, 'layers-3', `${m.stockUnits.toLocaleString()} aggregate stock units`)}${metric('Pending withdrawals', m.pendingWithdrawals, 'landmark', 'Requires review')}${metric('Pending KYC', m.pendingKyc, 'shield-check', `${m.pendingPayments} payment sessions pending`)}</div><div class="dashboard-grid"><div>${panel('Money movement', `<div class="trend-chart">${chart}</div><div class="legend"><span><i></i>Deposits</span><span><i class="purchase"></i>Purchases</span><span><i class="redemption"></i>Redemptions</span></div>`)}${panel('Financial totals', `<div class="detail-grid"><div class="detail-cell"><span>Wallet deposits</span><strong>${money(m.walletDepositVolume)}</strong></div><div class="detail-cell"><span>Card purchases</span><strong>${money(m.purchaseVolume)}</strong></div><div class="detail-cell"><span>Redemption credits</span><strong>${money(m.redemptionVolume)}</strong></div><div class="detail-cell"><span>Withdrawal requests</span><strong>${money(m.withdrawalVolume)}</strong></div></div>`, 'mt-16')}</div><div>${panel('Needs attention', `<div class="attention-list"><div class="attention-item" data-go="withdrawals"><i data-lucide="landmark"></i><div><strong>Withdrawals awaiting review</strong><span>Open the withdrawal queue</span></div><b class="attention-count">${m.pendingWithdrawals}</b></div><div class="attention-item" data-go="kyc"><i data-lucide="shield-check"></i><div><strong>KYC submissions awaiting review</strong><span>Review identity documents</span></div><b class="attention-count">${m.pendingKyc}</b></div><div class="attention-item" data-go="deposits"><i data-lucide="arrow-down-to-line"></i><div><strong>Payment sessions to reconcile</strong><span>Compare Site A references</span></div><b class="attention-count">${m.pendingPayments}</b></div></div>`)}${panel('Recent activity', `<div class="attention-list">${data.recent.transactions.slice(0, 5).map(item => `<div class="attention-item" ${rowClick(item.id, 'transaction')}><i data-lucide="receipt-text"></i><div><strong>${escape(item.reason)}</strong><span>${escape(item.userName)} · ${escape(item.reference)}</span></div><b class="amount">${money(item.amount)}</b></div>`).join('') || '<div class="empty">No activity yet.</div>'}</div>`, 'mt-16')}</div></div>`;
    }
    ...
    function renderSettings() { const s = state.summary?.settings || {}; return pageHeading('Settings', 'Current operational configuration. Editing is intentionally disabled.') + `<div class="settings-grid">${Object.entries({ 'Minimum deposit': money(s.minDeposit), 'Minimum withdrawal': money(s.minWithdrawal), 'Daily purchase limit': `${s.dailyPurchaseLimit} per price`, 'Operational charge': `${Number(s.operationalChargeRate || 0) * 100}%`, 'KYC bypass fee': money(s.kycBypassFee), 'Reward range': `${s.rewardMultiplierMin}× – ${s.rewardMultiplierMax}×`, 'Payment service': s.paymentServiceConfigured ? 'Configured' : 'Not configured' }).map(([label, value]) => `<div class="setting"><span>${escape(label)}</span><strong>${escape(value)}</strong></div>`).join('')}</div><div class="panel" style="margin-top:16px"><div class="panel-heading"><h3>Financial safety</h3></div><p style="color:var(--muted);line-height:1.7;margin:0">Manual wallet credits are available through the dedicated audited workflow. Direct balance editing, payment status editing, transaction editing, and environment-secret editing remain unavailable in this console.</p></div>`; }
    function renderReconciliation() { return pageHeading('Payment reconciliation', 'Recheck a customer payment from the server and credit the wallet only after Paystack confirms the matching transaction.') + `<section class="panel"><div class="panel-heading"><h3>Recheck a top-up</h3></div><p style="margin:0 0 17px;color:var(--muted);line-height:1.7">Enter the Site A deposit reference, transaction ID, or Paystack reference reported by the customer. The backend will ask Site B to verify the payment; already-credited deposits are safely ignored.</p><form id="paymentReconciliationForm"><label style="display:block;color:var(--muted);font-size:11px;font-weight:800">Payment reference<input id="reconciliationReference" required autocomplete="off" placeholder="DEP-… or Paystack reference" style="display:block;width:100%;max-width:560px;margin-top:7px" /></label><div style="display:flex;align-items:center;gap:12px;margin-top:15px"><button class="primary-button" id="reconcilePayment" type="submit">Recheck payment</button><span id="reconciliationResult" style="color:var(--muted);font-size:11px"></span></div></form></section><section class="panel" style="margin-top:16px"><div class="panel-heading"><h3>How to use this</h3></div><p style="margin:0;color:var(--muted);line-height:1.7">Open Deposits to find the customer and copy the Site A or Paystack reference, then run the recheck here. The action is recorded in the admin audit log and never credits a wallet twice.</p><button class="secondary-button" style="margin-top:16px" data-go="deposits">View deposits</button></section>`; }
    function renderWalletCredits() { return pageHeading('Manual wallet credit', 'Credit a customer’s wallet when a verified payment did not complete the automatic top-up.') + `<section class="panel"><div class="panel-heading"><h3>Credit wallet balance</h3></div><p style="margin:0 0 17px;color:var(--muted);line-height:1.7">Use this only after checking the payment in Paystack and/or the payment-service database. Every credit creates a wallet transaction, receipt, and audit-log entry. The customer’s password, redeemed balance, and payment flow are not changed.</p><form id="manualWalletCreditForm" style="max-width:620px"><label>User ID, email, or phone<input id="manualCreditIdentifier" required autocomplete="off" placeholder="usr_… or customer email / phone" /></label><label>Amount (GHS)<input id="manualCreditAmount" required type="number" min="0.01" max="100000" step="0.01" inputmode="decimal" placeholder="0.00" /></label><label>Reason or note<textarea id="manualCreditNote" required rows="4" maxlength="500" placeholder="For example: Paystack reference and confirmation checked by admin."></textarea></label><div style="display:flex;align-items:center;gap:12px;margin-top:15px"><button class="primary-button" id="manualCreditSubmit" type="submit">Credit wallet</button><span id="manualCreditResult" style="color:var(--muted);font-size:11px"></span></div></form></section><section class="panel" style="margin-top:16px"><div class="panel-heading"><h3>Safety check</h3></div><p style="margin:0;color:var(--muted);line-height:1.7">Confirm the exact user and amount before submitting. Repeating the same browser submission is safe, but a new submission creates a new audited credit.</p></section>`; }
    ...
    function renderUserDrawer(data) { const user = data.user; const docs = data.kycSubmission?.documents || []; return `<p class="eyebrow">CUSTOMER ACCOUNT</p><h2 class="drawer-title">${escape(user.name)}</h2><p class="drawer-subtitle">${escape(user.id)} · ${escape(user.email || user.phone)}</p><div class="detail-grid"><div class="detail-cell"><span>Wallet Balance</span><strong>${money(user.walletBalance)}</strong></div><div class="detail-cell"><span>Redeemed Balance</span><strong>${money(user.redeemedBalance)}</strong></div><div class="detail-cell"><span>KYC</span><strong>${status(user.kycStatus)}</strong></div><div class="detail-cell"><span>Joined</span><strong>${date(user.createdAt)}</strong></div><div class="detail-cell"><span>Purchases</span><strong>${user.purchaseCount}</strong></div><div class="detail-cell"><span>Redemptions</span><strong>${user.redemptionCount}</strong></div></div><div class="drawer-section"><h3>Identity</h3><div class="detail-grid"><div class="detail-cell"><span>Email</span><strong>${escape(user.email || '—')}</strong></div><div class="detail-cell"><span>Phone</span><strong>${escape(user.phone || '—')}</strong></div><div class="detail-cell"><span>Paystack assignment</span><strong>${escape(user.paystackPaymentEmail || '—')}</strong></div><div class="detail-cell"><span>Active sessions</span><strong>${user.sessionCount}</strong></div></div></div><div class="drawer-section"><h3>KYC documents</h3><div class="doc-list">${docs.length ? docs.map(doc => `<a class="doc-link" target="_blank" rel="noreferrer" href="/api/admin/users/${encodeURIComponent(user.id)}/kyc/documents/${encodeURIComponent(doc.storageKey)}"><span>${escape(doc.name)}</span><strong>${escape(doc.type)}</strong></a>`).join('') : '<p style="color:var(--muted)">No stored documents.</p>'}</div></div><div class="drawer-section"><h3>Recent financial activity</h3>${data.transactions.slice(0, 8).map(item => `<div class="recon-line"><span>${escape(item.reason)}</span><strong>${item.type === 'debit' ? '−' : '+'}${money(item.amount)}</strong></div>`).join('') || '<p style="color:var(--muted)">No transactions.</p>'}</div>`; }
    function renderReconDrawer(data) { const s = data.siteA, b = data.siteB, p = data.paystack; return `<p class="eyebrow">PAYMENT RECONCILIATION</p><h2 class="drawer-title">${money(s.amount)}</h2><p class="drawer-subtitle">${escape(s.transactionId || s.id)} · ${escape(s.userName)}</p><div class="split-grid"><div class="recon-card"><h4>Site A</h4>${reconLine('Status', status(s.status))}${reconLine('Reference', s.reference)}${reconLine('Amount', money(s.amount))}${reconLine('Expiry', date(s.expiresAt))}</div><div class="recon-card"><h4>Site B</h4>${reconLine('Status', status(b.status))}${reconLine('Transaction', b.transactionId || '—')}${reconLine('Availability', b.availableThrough)}</div><div class="recon-card"><h4>Paystack</h4>${reconLine('Status', status(p.status))}${reconLine('Reference', p.reference || '—')}${reconLine('Amount minor', p.amountMinor ? String(p.amountMinor) : '—')}${reconLine('Currency', p.currency)}</div></div>`; }
    ...
    document.addEventListener('submit', async event => {
        if (event.target.id === 'paymentReconciliationForm') {
            event.preventDefault();
            const reference = $('#reconciliationReference').value.trim();
            const button = $('#reconcilePayment'); const result = $('#reconciliationResult');
            if (!reference) return;
            button.disabled = true; result.textContent = 'Checking Paystack and the payment database…'; result.style.color = 'var(--muted)';
            try {
                const data = await api(`/api/admin/deposits/${encodeURIComponent(reference)}/reconcile`, { method: 'POST', body: '{}' });
                result.textContent = data.message || `Status: ${data.status}`; result.style.color = data.reconciled ? '#287953' : 'var(--muted)';
                toast(data.message || 'Payment recheck completed.');
            } catch (error) { result.textContent = error.message; result.style.color = 'var(--red)'; } finally { button.disabled = false; }
            return;
        }
        if (event.target.id !== 'manualWalletCreditForm') return;
        event.preventDefault();
        const button = $('#manualCreditSubmit'); const result = $('#manualCreditResult');
        button.disabled = true; result.textContent = 'Applying audited wallet credit…'; result.style.color = 'var(--muted)';
        try {
            const idempotencyKey = window.crypto?.randomUUID ? window.crypto.randomUUID() : `manual_${Date.now()}_${Math.random().toString(36).slice(2)}`;
            const data = await api('/api/admin/wallet-credits', { method: 'POST', body: JSON.stringify({ identifier: $('#manualCreditIdentifier').value.trim(), amount: $('#manualCreditAmount').value, note: $('#manualCreditNote').value.trim(), idempotencyKey }) });
            result.textContent = `${data.message} New wallet balance: ${money(data.user.walletBalance)}.`; result.style.color = '#287953';
            toast(data.message || 'Wallet credited.');
            $('#manualCreditAmount').value = ''; $('#manualCreditNote').value = '';
        } catch (error) { result.textContent = error.message; result.style.color = 'var(--red)'; } finally { button.disabled = false; }
    });
